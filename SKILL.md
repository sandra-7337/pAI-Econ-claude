...
2. **Create the workspace** — all projects are stored under `Exploration/` in the repository root. Follow these steps:
   ...
   a. **Determine the next project number**: scan `Exploration/` for existing subdirectories matching `Project_NNN_*` using shell `find` or `ls` — do NOT rely on a glob-only file-discovery operation, which has returned false negatives on this repo. Take the highest NNN found and add 1. If none exist, start at 001. Zero-pad to 3 digits.
...
For each paper identified in Stage 2, use the host's available web-search or page-fetch capability to confirm:
...
- Note: if the research involves international trade, trade policy, tariffs, global value chains, supply chains, firm-to-firm trade, buyer-seller matching, production networks, dynamic adjustment, spatial trade, or trade-related welfare analysis, recursively check every model card under `model_library/international_trade/` plus the retained root trade cards named in `model_library/international_trade/INDEX.md`; use `INDEX.md` only for routing and exclude it from model-family scoring
- Note: whenever the international-trade route is activated, jointly check `model_library/search-models.md`, `model_library/matching-models.md`, `model_library/dynamic-optimization-bellman.md`, and `model_library/general-equilibrium-basics.md`. The Stage 3b output must include a Required Library Scan Record and explain why any of these general cards is not applicable
- Note: the recommended trade baseline must report its core mechanism, minimum data requirements, and welfare-analysis path in addition to the normal primitives-inheritance handoff
...
2. **⚠️ REFERENCE VERIFICATION GATE — mandatory before writing any bibliography.**
   ...
   b. For each citation, use the host's available web-search capability to verify it exists. Specifically confirm:
...
3. **Generate the manuscript PDF** by writing a complete LaTeX file and compiling with pdflatex:
   ...
   \author{%
     ...
     \small <ACTUAL MODEL AND HOST>%
   }
   \date{Draft: <Month DD, YYYY>}
   ```

   **⚠️ Model attribution:** Replace `<ACTUAL MODEL AND HOST>` with the model and host actually running this pipeline session (for example, `Claude ... / Claude Code` or `GPT-... / Codex`), as stated in the current system context. NEVER hardcode or copy attribution from an earlier project — a paper generated by one model or host must not be attributed to another.

   The paper body should contain real academic prose: Introduction, Model (with subsections), Results (Propositions/Lemmas with proofs or proof sketches), Comparative Statics, Welfare, Boundary Cases, Testable Predictions, and a `thebibliography` section. Do NOT write a skeleton outline or include meta-commentary. See `templates/author_style_guide_econ.md` for the maintained writing-style rules.
